function minimum = SA3(filename,highclouds,IDX,anticlouds,ntarget,reg_NUM,im, options)
...
if ~nargin %user wants default options, give it and stop
    minimum = def;
    return
elseif nargin<6, %user gave only objective function, throw error
...
consec = 0;
T = Tinit;
%parent = randi([1 ntarget],1,reg_NUM);
parent = IDX;
initenergy =  ComputeEnergy(parent,highclouds,im,anticlouds);
oldenergy = initenergy;
total = 0;
a = 1;
if report==2, fprintf(1,'\n  T = %7.5f, oldenergy = %10.5f\n',T,oldenergy); end
...
while ~finished;
    ...
    if itry >= max_try || success >= max_success;
        if T < minT || consec >= max_consec_rejections;
            ...
            if report==2, % output
                fprintf(1,'  T = %7.5f, oldenergy = %10.5f\n',T,oldenergy);
                temp_T(a) = T;
                temp_E(a) = oldenergy;
                a = a + 1;
            end
    ...
    new_ntarget = randi(ntarget,1);
    new_index = randi(reg_NUM,1);
    newparam = current;
    newparam(new_index) = new_ntarget;
    newenergy = ComputeEnergy(newparam,highclouds,im,anticlouds);
